...
extension UIAlertController {
    ...
    /// This method is used to add a picker view and pop up in the screen
    ///
    /// - Parameters:
    ///   - type: type from Kind enumerator selection
    ///   - selection: Tnis closure is to be called after a selection is made
    func addLocalePicker(type: LocalePickerViewController.Kind, selection: @escaping LocalePickerViewController.Selection) {
        ...
        let buttonSelect: UIAlertAction = UIAlertAction(title: "Done", style: .default) { action in
            selection(info)
            print(info?.currencySymbol as Any)
        }
    ...
    func addAction(image: UIImage? = nil, title: String, color: UIColor? = nil, style: UIAlertAction.Style = .default, isEnabled: Bool = true, handler: ((UIAlertAction) -> Void)? = nil) {
        ...
        // button image
        if let image = image {
            action.setValue(image, forKey: AppConstants.Defaults.Key.image)
        }
        
        // button title color
        if let color = color {
            action.setValue(color, forKey: AppConstants.Defaults.Key.titleTextColor)
        }
    ...
    /// This method is used to set title for the picker view
    ///
    /// - Parameters:
    ///   - font: indicates title font
    ///   - color: indicates title text color
    func setTitle(font: UIFont, color: UIColor) {
        guard let title = self.title else { return }
        let attributes: [NSAttributedString.Key: Any] = [.font: font, .foregroundColor: color]
        let attributedTitle = NSMutableAttributedString(string: title, attributes: attributes)
        setValue(attributedTitle, forKey: AppConstants.Defaults.Key.attributedTitle)
    }
    ...
    /// Set message with font and color
    ///
    /// - Parameters:
    ///   - font: alert message font
    ///   - color: alert message color
    func setMessage(font: UIFont, color: UIColor) {
        ...
        setValue(attributedMessage, forKey: AppConstants.Defaults.Key.attributedMessage)
    }
    ...
    func set(vc: UIViewController?, width: CGFloat? = nil, height: CGFloat? = nil) {
        ...
        setValue(vc, forKey: AppConstants.Defaults.Key.contentViewController)
